...
Переносимый CS2 dedicated server с собственным C#-плагином, PostgreSQL и локальной Vue/PrimeVue-панелью. Проект запускается через Docker и не требует локальных SteamCMD, .NET SDK или Node.js.
...
- PostgreSQL для игроков, банов и назначений;
- JSON fallback до первого ответа control plane.

Плагин меняет внешний вид уже существующего оружия и поддерживает выбор модели/скина ножа. Он не выдаёт предметы и не меняет экономику матча. Перчатки, агенты, StatTrak и name tags пока не реализованы.
...
Первая команда поднимает PostgreSQL и админку. Вторая один раз создаёт остановленный контейнер игры. Затем откройте `http://localhost:8080`, войдите, выберите параметры и нажмите «Запустить».

Первый старт долгий: SteamCMD скачивает примерно 60–70 GB. Данные сохраняются во внешнем volume `cs2_cs2-data`. После загрузки карты подключитесь:
...
docker logs -f verona-server
docker compose down
docker compose build admin
docker compose --profile game create --build --force-recreate cs2
```

Не используйте `docker compose down -v` и не удаляйте `cs2_cs2-data`, если не хотите повторно скачивать CS2.
