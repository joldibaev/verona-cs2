...
- ножи поддерживаются через ограниченный allow-list item definition index и `ChangeSubclass`; перчатки, агенты, StatTrak и name tags пока не поддерживаются.
...
- `admin/ui` — Vue 3 + TypeScript + Vite + PrimeVue SPA.
...
- Backend является единственным владельцем авторизации и проверки ролей. Router guards и скрытые кнопки в Vue — UX, не security boundary.
...
10. На SteamID может быть выбран только один `weapon_knife_*`/`weapon_bayonet`: backend удаляет прежнюю модель перед upsert, а плагин принимает только известные item definition index.
...
Skin collections — пользовательские snapshots loadout в `skin_collections` и `skin_collection_items`. Активна максимум одна коллекция на SteamID. Переключение атомарно заменяет `player_weapon_skins` и ставит `refresh_skins`; self-service изменения синхронизируют активную коллекцию.

- Перчатки и агенты требуют отдельной подмены item definition/model и пока не реализованы. Ножи реализованы, но после обновлений CS2 требуют отдельного игрового smoke-теста модели, анимаций и paint kit.
