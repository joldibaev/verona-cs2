...
10. На SteamID может быть выбран только один `weapon_knife_*`/`weapon_bayonet` для каждого scope (`both`, `ct`, `t`): backend удаляет прежнюю модель в том же scope, а плагин принимает только известные item definition index.
...
Skin collections — пользовательские snapshots loadout в `skin_collections` и `skin_collection_items`. Оружейные назначения имеют scope `both`/`ct`/`t`; оружие одной команды нельзя назначить другой. Активна максимум одна коллекция на SteamID. Переключение атомарно заменяет `player_weapon_skins` и ставит `refresh_skins`; self-service изменения синхронизируют активную коллекцию.
