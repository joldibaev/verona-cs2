...
public sealed class AdminApiClient : IDisposable
{
    ...
    private async Task RefreshPlayerSkinsAsync(ulong steamId, Action afterUpdate)
    {
        try
        {
            var values = await _http.GetFromJsonAsync<SkinDto[]>($"/api/plugin/players/{steamId}/skins") ?? [];
            Server.NextFrame(() =>
            {
                _skins.UpdatePlayerSkins(steamId, values.Select(x => new KeyValuePair<string, SkinDefinition>(x.Weapon, new(x.PaintKit, x.Wear, x.Seed))));
                afterUpdate();
            });
        }
        catch (Exception exception)
    ...
    private sealed record SkinDto(string Weapon, int PaintKit, float Wear, int Seed);
}
